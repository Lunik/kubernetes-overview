;(function () {
  ...
  function updateDiagram () {
    d3.json('/cluster.json', function (json) {
      var cluster = prepareD3(json.links)
      var currentCluster = $.extend({}, Graph.cluster)
      ...
      for (var index in cluster.nodes) {
        ...
        var result = $.grep(currentCluster.nodes, function (e) { return e.id == value.id })
        if (result.length == 0) {
          todo.add.nodes.push(value)
        }
      }
      ...
      for (var index in cluster.links) {
        ...
        var result = $.grep(currentCluster.links, function (e) { return e.source.id == value.source.id && e.target.id == value.target.id })
        if (result.length == 0) {
          todo.add.links.push(value)
        }
      }
