  ...
  function updateDiagram () {
    d3.json('/cluster.json', function (json) {
      ...
      for (var index in cluster.nodes) {
        var value = cluster.nodes[index]

        var nodes = Graph.force.nodes()

        var result = $.grep(nodes, function (e) { return e.id == value.id })
        if (result.length == 0) {
          nodes.push(value)
        }
      }

      for (var index in cluster.links) {
        var value = cluster.links[index]

        var links = Graph.force.links()

        var result = $.grep(links, function (e) { return e.source.id == value.source.id && e.target.id == value.target.id })
        if (result.length == 0) {
          var source = $.grep(links, function (e) { return e.source.id == value.source.id})

          if (source.length > 0) {
            value.source = source[0].source
          }

          var target = $.grep(links, function (e) { return e.target.id == value.target.id})
          if (target.length > 0) {
            value.target = target[0].source
          }
